...
void janus_videoroom_incoming_data(janus_plugin_session *handle, char *buf, int len) {
    ...
    if(session->participant_type == janus_videoroom_p_type_publisher) {
        ...
        *(text + len) = '\0';
        JANUS_LOG(LOG_VERB, "Got a DataChannel message (%zu bytes) to forward: %s\n", strlen(text), text);
        if(current_listener && current_listener->data && current_listener->feed){
            janus_videoroom_participant *participant = (janus_videoroom_participant *)current_listener->feed;
            /* Save the message if we're recording */
            janus_recorder_save_frame(participant->drc, text, strlen(text));
            if(participant != NULL && participant->data && participant->session && participant->session->started && participant->session->handle) {
            ...
            if(participant != NULL && participant->listeners) {
                ...
                while(ps) {
                    ...
                    if(l && l->session && current_listener->session && l->session != current_listener->session) {
                        if( l->data && !l->paused && l->session->started && l->session->handle) {
